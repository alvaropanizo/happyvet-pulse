...
test("selects a file, scans it, and shows structured record card", async ({ page }) => {
  ...
  await expect(page.getByLabel("Reset working page")).toBeVisible();
});
...
test("allows cancel/confirm reset after successful scan", async ({ page }) => {
  await page.route("**/api/v1/documents/scan", async (route) => {
    await route.fulfill({
      status: 200,
      contentType: "application/json",
  ...
  await page.getByLabel("Reset working page").click();
  await expect(page.getByText("You will loose changes in your working page, are you sure")).toBeVisible();
